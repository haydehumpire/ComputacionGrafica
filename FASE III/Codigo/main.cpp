...
Button Button0 = { 600,400, 200,70, "Borrar"};
Button Button1 = { 600,100, 200,70, "Dibujar" };
Button Button2 = { 600,200, 80,80, "+" };
Button Button3 = { 750,200, 80,80, "-" };
...
static void display(void)
{
	...
	butonCiz(&Button0);
	butonCiz(&Button1);
	ekranaYaz(GLUT_BITMAP_HELVETICA_10,"Quad Tree", 600, 300);
	butonCiz(&Button2);
	butonCiz(&Button3);
...
void selectionSort(vector<point>  toSort)
{
	...
	cout << " Encontradas (" << toSort.size() << " num ) :\n";
	...
	case GLUT_LEFT_BUTTON:
		...
		case GLUT_DOWN:
			if (yeniNokta.x > -quadW && yeniNokta.x < quadW && yeniNokta.y < quadH && yeniNokta.y>-quadH) {
				...
				if (ButtonClickTest(&Button0, yeniNokta.x, yeniNokta.y)) {
					delete qtree;
					qtree=new QuadTree <int>(merkez, eksen, kumeBoyutu);
					tumNoktalar.clear();
					bulunanNoktalar.clear();
